...
# creates figure
fig = plt.figure()
ax = fig.add_axes([0,0.3588,1,0.6412]) # get rid of white border
if not options.show_axes:
    fig1 = plt.gca()
    fig1.axes.get_xaxis().set_visible(False)
    fig1.axes.get_yaxis().set_visible(False)
ax2 = fig.add_axes([0,0,1,0.3588]) # get rid of white border
fig2 = plt.gca()
...
#                    person.walk_faster(horizontal_walk_time)
    ...
    monitor.monitor_pedestrians(monitor_pedestrian_state, observer)
...
t1 = time.time()
interval = (t1 - t0)
ani = animation.FuncAnimation(fig, animate, frames=int(options.duration/options.dt), interval=interval, blit=False, repeat=False) # by default the animation function loops so set repeat to False in order to limit the number of frames generated to num_frames
